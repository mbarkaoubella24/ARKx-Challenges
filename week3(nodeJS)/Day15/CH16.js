const fetchWeatherData = async (lat, lng) => {
    try {
        const response = await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lng}&current_weather=true`);
        if (!response.ok) {
            throw new Error('Fetching Data Failed !!'); 
        }

        const data = await response.json();
        const temperature = data.current_weather.temperature;
        const temperature_unit = data.current_weather_units.temperature;

        const currentTemperature = `${temperature} ${temperature_unit}`; 
        return currentTemperature;
    } catch (error) {
        throw new Error('Error fetching weather data: ' + error.message); 
    }
}

const checkForCity = async (cityName) => {
    try {
        const cities = [
            { name: 'New York', lat: 40.7128, lng: -74.0060 },
            { name: 'London', lat: 51.5074, lng: -0.1278 },
            { name: 'Paris', lat: 48.8566, lng: 2.3522 },
            { name: 'Tokyo', lat: 35.6895, lng: 139.6917 },
            { name: 'Sydney', lat: -33.8651, lng: 151.2099 },
            { name: 'Rome', lat: 41.9028, lng: 12.4964 },
            { name: 'Cairo', lat: 30.0444, lng: 31.2357 },
            { name: 'Rio de Janeiro', lat: -22.9068, lng: -43.1729 },
            { name: 'Dubai', lat: 25.2048, lng: 55.2708 },
            { name: 'Rabat', lat: 34.0209, lng: -6.8416 }
        ];
        const cityFound = cities.find(c => c.name.toLowerCase() === cityName.toLowerCase()); 
        if (cityFound) {
            const { lat, lng } = cityFound;
            const info = await fetchWeatherData(lat, lng);
            return info;
        } else {
            return `City not found`; 
        }
    } catch (error) {
        throw new Error('Error checking for city: ' + error.message); 
    }
}

module.exports = {
    checkForCity,
    fetchWeatherData 
}

const http = require('http');

const server = http.createServer(async (req, res) => {
    try {
        const parsedUrl = new URL(req.url, `http://${req.headers.host}`); 
        const path = parsedUrl.pathname;
        const query = parsedUrl.searchParams;
        if (path === `/weather`) {
            let city = query.get('city'); 
            const cityInfo = await checkForCity(city);
            res.writeHead(200, { 'Content-Type': 'text/plain' })
                .end(`The Weather Information for ${city}: ${cityInfo} `);
        } else {
            res.writeHead(404, { 'Content-Type': 'text/plain' })
                .end('This endpoint does not exist'); 
        }
    } catch (error) {
        console.error(error); 
        res.writeHead(500, { 'Content-Type': 'text/plain' })
            .end('Internal Server Error'); 
    }
});

server.listen(3000, () => {
    console.log('The server is listening on port 3000');
})

